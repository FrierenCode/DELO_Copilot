...
PRD v2 기준에서 이 제품은 "AI가 한번 도와주는 툴"이 아니라 문의 접수부터 저장, 후속 관리까지 이어지는 운영 시스템입니다.
...
- 문의 텍스트 정리, 해시 생성, 사용자 스코프 dedup, 전역 parse cache
- 계약 조건 누락/리스크를 요약하는 체크 엔진
- 크리에이터 프로필 기반 견적 계산 엔진
- 템플릿 기반 답장 초안 생성
- Pro 플랜 전용 Negotiation AI API
- inquiry history/detail 조회와 reply draft 수정 API
- Supabase 기반 inquiries, deals, checks, drafts, profiles 저장 계층
...
- `ReplyCard`에서 초안을 직접 편집할 수 있고, inquiry detail 화면에서는 `PATCH /api/inquiries/[id]`로 저장됩니다.
...
- 상단 네비게이션은 `Home`, `Parse`, `History`, `Settings`를 제공합니다.
- `ParseInterface`는 source type 선택, 문의 입력, parse 결과 카드 렌더링을 담당합니다.
- 결과 카드는 `InquiryCard`, `QuoteCard`, `ChecksCard`, `ReplyCard`로 분리되어 있습니다.
- History는 인증 사용자 기준 최근 inquiry 50개까지 조회합니다.
...
- `GET/PATCH /api/inquiries/[id]`를 통한 inquiry 상세 조회 및 reply draft 수정
...
- `reply-template-service`: Negotiation AI fallback 텍스트 생성
...
- `types/parse-api.ts`: 프론트엔드가 consume하는 parse/inquiry 응답 타입 정의
...
현재 UI는 제품 완성본이 아니라 API와 백엔드 상태를 확인하기 위한 최소 워크스페이스 수준입니다.
...
- 인증되지 않은 경우 빈 배열을 반환합니다.
- History 화면은 이 API를 사용합니다.
...
- 저장된 inquiry의 `parsed_json`, `missing_fields`를 읽습니다.
- 현재 사용자 플랜 기준으로 quote/checks/reply draft를 다시 구성합니다.
- `reply_drafts_json`에 저장된 수정본이 있으면 생성 결과 위에 merge합니다.

### `PATCH /api/inquiries/[id]`

reply draft 편집 결과를 inquiry에 저장합니다.
...
- `reply_drafts_json` 컬럼으로 inquiry 단위 draft 수정본을 저장합니다.
...
현재 저장소는 "문의 파싱 + inquiry 저장 + 딜 저장 API + 플랜 gate + 운영 저장소 + 최소 UI"까지 연결된 MVP입니다.
...
- 실제 운영용 Deals Dashboard 프론트엔드 완성
- 설정 화면 기능 구현
- 결제/Billing
- 보다 강한 운영용 E2E 검증
