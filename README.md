...
- Supabase 기반 딜 저장, 체크 저장, 답장 초안 저장, 상태 로그 저장
- Free/Pro 사용량 제한과 대시보드 알림 집계 로직
- PostHog 이벤트 추적과 Sentry 기반 서버 오류 보고
...
- 파싱은 LLM 기반으로 동작하며 모델 폴백 구조를 가집니다.
- 응답이 비어 있거나, JSON 코드 펜스로 감싸져 있거나, 스키마 검증에 실패하는 경우를 별도로 처리합니다.
- 파싱 결과는 정규화 후 스키마 검증을 거칩니다.
...
- 협상형 답장은 LLM을 우선 사용합니다.
- LLM 응답이 비어 있거나 실패하면 템플릿 기반 협상 답장으로 폴백합니다.
...
이번 단계에서 아래 서버 계층이 추가되었습니다.

- `POST /api/deals`를 통한 딜 저장
- `GET /api/deals`를 통한 딜 목록 및 알림 집계 조회
...
- `deal-service`: 파싱 결과를 딜 저장용 payload로 조합
- `repositories`: 딜, 체크, 답장 초안, 상태 로그 저장 계층
- `usage-guard`: 플랜별 사용량 제한 처리
- `alert-engine`: 대시보드 알림 수치 계산
...
```json
{
  "raw_text": "안녕하세요. 유료 틱톡 협업 가능 여부를 문의드립니다.",
  "source_type": "dm",
...
`parsed_json`은 전달되더라도 서버에서 다시 검증합니다.
...
현재 상태는 "개발 가능한 기반"을 넘어, PRD에서 말하는 핵심 운영 흐름의 일부를 이미 코드로 옮긴 단계입니다. 특히 아래 항목은 이번에 추가되었거나 구체화되었습니다.

- 협찬 건 저장과 이력 관리를 위한 Supabase 마이그레이션
- 딜, 체크, 답장 초안 저장을 위한 repository 계층
- 사용량 제한과 paywall 트래킹 구조
- Sentry, PostHog, 구조화 로그 기반 운영 관측 구조

다만 아직 아래 영역은 더 필요합니다.

- 실제 사용자 인증과 크리에이터 프로필 저장의 완성
- Deals Dashboard UI
- 알림 이후의 후속 운영 자동화
- Billing과 Pro 플랜 정책 정교화
- 프롬프트 안정화 및 품질 검증 자동화
