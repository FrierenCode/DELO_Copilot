...
- 먼저 사용자별 `inquiries` 중복 조회를 수행하고, 없으면 전역 `parse_cache`를 조회합니다.
- 전역 `parse_cache` hit 시에도 현재 요청용 canonical `inquiry` 레코드를 생성하지만 LLM은 다시 호출하지 않습니다.
- 두 캐시 모두 미스일 때만 LLM을 호출하고, 결과를 `inquiries`와 `parse_cache`에 저장합니다.
...
- `GET /api/deals`를 통한 딜 목록, status filter, alert 집계 조회
- `GET/PATCH /api/deals/[id]`를 통한 딜 상세 조회 및 상태/메모 수정
- `GET /api/deals/alerts`를 통한 운영 alert item 조회
- `GET/POST /api/creator-profile`를 통한 크리에이터 프로필 조회/저장
...
인증된 사용자의 저장된 딜 목록과 알림 요약을 반환합니다. `?status=Lead|Replied|Negotiating|Confirmed|Delivered|Paid|ClosedLost` 필터를 지원합니다.
...
Free 플랜은 alert 기능이 차단되고, 응답에는 0값 요약과 빈 `items` 배열이 반환됩니다.

### `GET /api/deals/[id]`

단일 딜과 연결된 체크, 답장 초안, 상태 로그를 반환합니다.

### `PATCH /api/deals/[id]`

단일 딜의 상태, 메모, 다음 액션, 일정 관련 필드를 수정합니다. 상태 변경은 서버 측 상태 전이 규칙을 검증합니다.

### `GET /api/deals/alerts`

Pro 플랜 전용 운영 alert 엔드포인트입니다. 요약 count뿐 아니라 구조화된 `items` 배열을 함께 반환합니다.

### `GET /api/creator-profile`

인증된 사용자의 크리에이터 프로필을 반환합니다.
...
- PRD 기준 메서드는 `POST`입니다.
- 기존 클라이언트 호환을 위해 `PUT /api/creator-profile`도 현재는 alias로 유지됩니다.
...
- `parse_cache`는 전역 비용 최적화용 보조 캐시이며, hit 시에도 요청별 canonical `inquiry`를 다시 생성합니다.
- 현재 저장소에는 `subscriptions` 테이블이 없으므로 플랜의 canonical source of truth는 `user_plans`입니다.
...
3. 현재 저장소 기준 `user_plans` 기반 Free/Pro 플랜 정책
