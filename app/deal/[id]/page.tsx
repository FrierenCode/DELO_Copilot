...
export default function DealDetailPage() {
  ...
  return (
    <div className="flex flex-col">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-xs text-[var(--d-f)] mb-6">
        <Link href="/dashboard/history" className="hover:text-[var(--d-m)] transition-colors">
          히스토리
        </Link>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <polyline points="9,18 15,12 9,6" />
        </svg>
        <span className="text-[var(--d-m)]">{p.brand_name} 문의 상세</span>
      </nav>

      {/* Header Card */}
      <header className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] rounded-xl p-6 mb-8 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-[var(--d-border)] border border-[var(--d-border)] flex items-center justify-center text-indigo-400 font-bold text-2xl flex-shrink-0" style={{ boxShadow: "2px 2px 0 rgba(99,102,241,0.3)" }}>
        ...
        <div className="flex flex-col items-end gap-2">
          <Link
            href="/dashboard"
            className="btn-gradient text-white px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all shadow-lg shadow-[#6366F1]/20"
          >
            Deal로 저장
          ...
          <Link
            href="/dashboard"
            className="text-xs text-[var(--d-f)] hover:text-[var(--d-m)] underline underline-offset-4 transition-colors"
          >
            이미 저장된 딜 보기 →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-7 space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              {parseFields.map(({ label, value }) => (
                <div key={label} className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] p-4 rounded-xl">
                  <p className="text-[11px] text-[var(--d-f)] uppercase font-bold mb-1">{label}</p>
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] p-5 rounded-xl hover:border-[#6366F1]/30 transition-colors">
                <p className="text-xs text-[var(--d-f)] mb-1">Floor</p>
                <p className="text-xl font-bold text-[var(--d-b)]">{formatKRW(q.floor)}</p>
              </div>
              <div className="bg-[#6366F1]/5 border-2 border-[#6366F1] p-5 rounded-xl relative overflow-hidden shadow-xl shadow-indigo-500/10">
                <div className="absolute top-0 right-0 bg-indigo-500 text-[10px] font-bold px-2 py-0.5 text-white uppercase tracking-tighter rounded-bl-lg">
                  Recommended
                </div>
              ...
              <div className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] p-5 rounded-xl hover:border-[#6366F1]/30 transition-colors">
                <p className="text-xs text-[var(--d-f)] mb-1">Premium</p>
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] text-[var(--d-f)] italic">* DELO AI가 법적 검토를 보조합니다.</span>
            </div>

            <div className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] rounded-xl divide-y divide-[var(--d-border)] overflow-hidden">
              {sortedChecks.length === 0 && (
                <p className="p-4 text-sm text-[var(--d-f)]">체크 항목이 없습니다.</p>
              )}
              {sortedChecks.map((check, i) => {
                ...
                return (
                  <div key={check.check_code} className="p-4 flex items-center justify-between relative">
                    ...
                    {isBlurred ? (
                      <div className="absolute inset-0 flex items-center justify-center bg-[var(--d-surface)]/60">
                        <span className="bg-[var(--d-border)] text-[var(--d-f)] text-[10px] px-2 py-1 rounded border border-[var(--d-border)]">
                          PRO 전용 항목
                        </span>
                      </div>
                    ) : (
            ...
            {/* Free upgrade nudge below checklist */}
        ...
        <div className="col-span-12 lg:col-span-5 space-y-6">
          ...
          {/* Reply draft panel */}
          <section className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] rounded-xl flex flex-col">
            <div className="p-4 border-b border-[var(--d-border)]">
              ...
              <div className="flex gap-2 p-1 bg-[var(--d-surface2)] rounded-lg">
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        active
                          ? "bg-[var(--d-border)] text-[var(--d-h)] border border-[var(--d-border)]"
                          : locked
            ...
            <div className="relative p-4">
              ...
              <button
                ...
                onClick={handleCopy}
                title="복사"
                className="absolute top-6 right-6 p-2 rounded-lg bg-[var(--d-border)] border border-[var(--d-border)] text-[var(--d-m)] hover:text-[var(--d-h)] transition-colors z-10"
              >
                {copied ? (
              ...
              {!isPro && activeTab !== "polite" ? (
                <div className="h-40 flex flex-col items-center justify-center gap-2 text-center">
                  <p className="text-sm text-[var(--d-f)]">이 초안은 Standard 전용입니다.</p>
              ...
              <button
                ...
                className={[
                  ...
                  saveMsg === "저장됨"
                    ...
                    : "text-[var(--d-f)] hover:text-[var(--d-m)] border-transparent hover:border-[var(--d-border)]",
                ].join(" ")}
              >
                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Raw text toggle */}
          <section className="d-pixel-card bg-[var(--d-surface)] border border-[var(--d-border)] rounded-xl overflow-hidden">
            <button
              type="button"
            ...
            {rawOpen && (
              <div className="px-4 pb-4">
                {data.raw_text_preview ? (
                  <pre className="text-xs text-[var(--d-m)] leading-relaxed whitespace-pre-wrap bg-[var(--d-surface2)] rounded-lg p-3 max-h-48 overflow-y-auto">
                    {data.raw_text_preview}
                  </pre>
                ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <Link
                    ...
                    className="block w-full py-2 btn-gradient text-white text-[12px] font-bold rounded-lg transition-all text-center"
                  >
                    혜택 더보기
                  </Link>
                </div>
