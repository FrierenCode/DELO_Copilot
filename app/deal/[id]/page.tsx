  ...
  return (
    <div className="flex flex-col">
      ...
      <nav className="flex items-center gap-2 text-xs text-slate-500 mb-6">
        ...
        <span className="text-slate-300">{p.brand_name} 문의 상세</span>
      </nav>

      {/* Header Card */}
      <header className="bg-[#13131A] border border-[#1E1E2E] rounded-xl p-6 mb-8 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 rounded-full bg-slate-800 border border-[#1E1E2E] flex items-center justify-center text-indigo-400 font-bold text-2xl flex-shrink-0">
            {monogram(p.brand_name)}
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">{p.brand_name}</h1>
        ...
        <div className="flex flex-col items-end gap-2">
          <Link
            href="/dashboard"
            className="bg-indigo-500 hover:bg-indigo-400 text-white px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all shadow-lg shadow-indigo-500/20"
          >
            Deal로 저장
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-7 space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              {parseFields.map(({ label, value }) => (
                <div key={label} className="bg-[#13131A] border border-[#1E1E2E] p-4 rounded-xl">
                  <p className="text-[11px] text-slate-500 uppercase font-bold mb-1">{label}</p>
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-[#13131A] border border-[#1E1E2E] p-5 rounded-xl hover:border-slate-700 transition-colors">
                <p className="text-xs text-slate-500 mb-1">Floor</p>
                <p className="text-xl font-bold text-slate-300">{formatKRW(q.floor)}</p>
              </div>
              <div className="bg-[#13131A] border-2 border-indigo-500 p-5 rounded-xl relative overflow-hidden shadow-xl shadow-indigo-500/10">
                ...
                <p className="text-xl font-bold text-white">{formatKRW(q.target)}</p>
              </div>
              <div className="bg-[#13131A] border border-[#1E1E2E] p-5 rounded-xl hover:border-slate-700 transition-colors">
                <p className="text-xs text-slate-500 mb-1">Premium</p>
                <p className="text-xl font-bold text-slate-300">{formatKRW(q.premium)}</p>
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] text-slate-500 italic">* DELO AI가 법적 검토를 보조합니다.</span>
            </div>

            <div className="bg-[#13131A] border border-[#1E1E2E] rounded-xl divide-y divide-[#1E1E2E] overflow-hidden">
              {sortedChecks.length === 0 && (
                <p className="p-4 text-sm text-slate-500">체크 항목이 없습니다.</p>
              )}
              {sortedChecks.map((check, i) => {
                ...
                return (
                  <div key={check.check_code} className="p-4 flex items-center justify-between relative">
                    <span className={`text-sm text-slate-300 pr-4 ${isBlurred ? "blur-[3px] select-none" : ""}`}>
                      {check.message}
                    </span>
                    {isBlurred ? (
                      <div className="absolute inset-0 flex items-center justify-center bg-[#13131A]/40">
                        <span className="bg-slate-800 text-slate-400 text-[10px] px-2 py-1 rounded border border-[#1E1E2E]">
                          PRO 전용 항목
                        </span>
                      </div>
                    ) : (
            ...
            {/* Legal disclaimer */}
            <p className="mt-3 text-[11px] text-slate-600 leading-relaxed px-1">
        ...
        <div className="col-span-12 lg:col-span-5 space-y-6">
          ...
          {/* Reply draft panel */}
          <section className="bg-[#13131A] border border-[#1E1E2E] rounded-xl flex flex-col">
            <div className="p-4 border-b border-[#1E1E2E]">
              ...
              <div className="flex gap-2 p-1 bg-[#0A0A0F]/50 rounded-lg">
            ...
            <div className="relative p-4">
              ...
              <button
                ...
                onClick={handleCopy}
                title="복사"
                className="absolute top-6 right-6 p-2 rounded-lg bg-slate-800 border border-[#1E1E2E] text-slate-400 hover:text-white transition-colors z-10"
              >
                {copied ? (
              ...
              {!isPro && activeTab !== "polite" ? (
                ...
                <textarea
                  ...
                  rows={10}
                  className="w-full bg-transparent border-none text-slate-300 text-sm leading-relaxed focus:outline-none resize-none pr-10"
                  placeholder="초안 생성 중..."
                />
              )}
            </div>

            <div className="p-4 border-t border-[#1E1E2E] flex justify-end">
              <button
                type="button"
                onClick={handleSave}
                disabled={saving}
                className={[
                  ...
                  saveMsg === "저장됨"
                    ...
                    : "text-slate-500 hover:text-slate-300 border-transparent hover:border-[#1E1E2E]",
                ].join(" ")}
              >
                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {rawOpen && (
              <div className="px-4 pb-4">
                {data.raw_text_preview ? (
                  <pre className="text-xs text-slate-400 leading-relaxed whitespace-pre-wrap bg-[#0A0A0F] rounded-lg p-3 max-h-48 overflow-y-auto">
                    {data.raw_text_preview}
                  </pre>
                ) : (
                  <p className="text-xs text-slate-600 pb-2">원문 미리보기를 사용할 수 없습니다.</p>
                )}
              </div>
            )}
          </section>
            ...
            <div className="bg-gradient-to-br from-indigo-900/30 to-purple-900/30 border border-indigo-500/30 rounded-xl p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-bold text-white mb-1">PRO 업그레이드</h3>
